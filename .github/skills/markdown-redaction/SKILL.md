...
### Conventions spécifiques au projet ShelfScan
- Identifiants workstream : `WS1` à `WS5`
- Identifiants milestone : `M1` à `M4`
- Identifiants tâche : `#NNN` (3 chiffres)
- Identifiants gate : `G-Segment`, `G-OCR`, `G-Pipeline`, `G-Eval`
- Références spec : `§N` ou `§Étape N` (ex. `§3 Étape 4`, `§6 Métriques`)
- Chemins fichiers : backticks (`` `src/ocr.py` ``, `` `data/ground_truth/` ``)
- Modèles OCR : `PaddleOCR`, `TrOCR`, `Tesseract` (noms officiels)
...
- [ ] Acronymes définis à la première occurrence (CER, WER, OCR, CLAHE, etc.)
- [ ] Cohérence des identifiants (WS1..WS5, M1..M4, #NNN)
